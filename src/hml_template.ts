...
// Python 버전에서 추출한 HML XML 템플릿
export function getHeaderTemplate(
  bindataListStr: string,
...
<HWPML Style="embed" SubVersion="10.0.0.0" Version="2.91">
    <HEAD SecCnt="1">
        ...
        <MAPPINGTABLE>
            ...
            <BORDERFILLLIST Count="${4 + additionalBorderCount}">
    ...
    <BODY>
        <SECTION Id="0">
            <SECDEF CharGrid="0" FirstBorder="false" FirstFill="false" LineGrid="0" SpaceBetweenColumns="0" TabStop="8000" TextDirection="0" VerticalPageAlign="Top">
                ...
                <PAGEDEF GutterType="Left" Height="${margin.height}" Landscape="0" Width="${margin.width}">
                    <PAGEMARGIN Bottom="${margin.bottom}" Footer="${margin.footer}" Gutter="0" Header="${margin.header}" Left="${margin.left}" Right="${margin.right}" Top="${margin.top}"/>
                </PAGEDEF>
            </SECDEF>`;
}
